...
app.post('/api/persons', async(req, res, next) => {
  ...
  if (found) {
    ...
    try {
      ...
      // const err = { name: 'post-failed', ...error, number: body.number };
      next(error)
    }
  }
})
app.put('/api/persons/:id', async(req, res, next) => {
  ...
  try {
    const updatedPerson = await PersonModel.findOneAndUpdate(filter, infoToBeUpdated, { new: true, runValidators: true, context: 'query' }).exec()
...
app.delete('/api/persons/:id', async(req, res, next) => {
  ...
  try {
    ...
    if (deletedObject) {
      try {
        await PersonModel.findOneAndDelete(filter).exec()
        res.status(204).end()
      } catch (error) {
        next(error)
      }
