...
describe('there are 2 blogs initially', () => {
    ...
    test('the first Blog is about Learning Mern', async() => {
        ...
        expect(titles).toContain('Learning Mern');
    });
...
describe('sending post request', () => {
    test('Check that doing post increases the amount of blogs', async() => {
        ...
        let user3LoginResponse = await api
            .post('/api/login')
    ...
    test('Check if title and url are missing from the post request the response has status code 400', async() => {
        ...
        try {
            let user1LoginResponse = await api
                .post('/api/login')
    ...
    test('Check if a user does not have a valid token the response has status code 401', async() => {
        ...
        try {
            await api
                .post('/api/blogs')
                .set('Authorization', 'bearer ' + '5a3d5e3')
                .send(newBlog);
        } catch (e) {
...
describe('delete request', () => {
    test('attempting to delete a blog without a valid token the response has status code 401', async() => {
        ...
        const loginIncorrectUserResponse = await api
            .post('/api/login')
            .send({ username: incorrectUser.username, password: newUser2Password });
    ...
    test('Check that deleting a blog decreases the amount of blogs', async() => {
        ...
        const loginCorrectUserResponse = await api
            .post('/api/login')
            .send({ username: correctUser.username, password: newUserPassword });
    ...
    test('Check that deleting a blog with an invalid id returns status code 404', async() => {
        ...
        const loginUserResponse = await api
            .post('/api/login')
            .send({ username: user.username, password: newUserPassword });
