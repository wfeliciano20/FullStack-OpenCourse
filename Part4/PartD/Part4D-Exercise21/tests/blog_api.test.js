...
const newUser2Password = 'Doe345';
const newUser3Password = 'jackD123';
...
beforeEach(async() => {
    ...
    hash = bcrypt.hashSync(newUser2Password, salt);
    ...
    hash = bcrypt.hashSync(newUser3Password, salt);
...
describe('delete request', () => {
    test('attempting to delete a blog without a valid token the response has status code 401', async() => {
        const initialBlogsFromDB = await api.get('/api/blogs');
        const users = await api.get('/api/users');
        const incorrectUser = users.body[1];
        const loginIncorrectUserResponse = await api.post('/api/login').send({ username: incorrectUser.username, password: newUser2Password });
        const incorrectLoginResponse = JSON.parse(loginIncorrectUserResponse.text);
        const blogToBeDeletedId = initialBlogsFromDB.body[0].id;
        try {
            await api.delete(`/api/blogs/${blogToBeDeletedId}`).set('Authorization', 'bearer ' + incorrectLoginResponse.token);
        }catch (e) {
            expect(e.response.status).toBe(401);
    ...
    test('Check that deleting a blog decreases the amount of blogs', async() => {
        ...
        const users = await api.get('/api/users');
        const correctUser = users.body[0];
        const loginCorrectUserResponse = await api.post('/api/login').send({ username: correctUser.username, password: newUserPassword });
        const correctLoginResponse = JSON.parse(loginCorrectUserResponse.text);
        await api.delete('/api/blogs/' + ids[0]).set('Authorization', 'bearer ' + correctLoginResponse.token);
        const blogs = await api.get('/api/blogs');
        expect(blogs.body).toHaveLength(initialBlogs.length - 1);
    }, 10000);
    ...
    test('Check that deleting a blog with an invalid id returns status code 404', async() => {
        ...
        const loginUserResponse = await api.post('/api/login').send({ username: user.username, password: newUserPassword });
        const loginResponse = JSON.parse(loginUserResponse.text);
        try {
            await api.delete('/api/blogs/' + '5a3d5e3').set('Authorization', 'bearer ' + loginResponse.token);
        } catch (error) {
